...
適切なテンプレートを選べなかったファイルは「未選択」になる。人がテンプレートを選ぶと、
AI分類が有効な場合は自動で抽出が始まる。テンプレート未選択のまま承認すると業務メタデータは付けない。
AI分類をオフにした移行では、テンプレートと値の手動指定を利用する。

一覧の抽出状況は「抽出済み」「該当項目なし」「抽出失敗」「手動入力」を区別する。
「抽出済み」は全項目が埋まったことや正確性を保証する表示ではない。本文に値がない項目は空欄になる。
抽出失敗のファイルは「対応が必要」に表示し、一括承認の対象から外す。
「項目を確認・編集」から再抽出・手入力して個別承認するか、テンプレートを変更する。
抽出値はこの段階ではローカルの下書きであり、Boxへ付与するのは承認後。
