...
| 場所 | 設定するもの |
| --- | --- |
| Macの`.env` | Boxの認証、プロキシ・CA、必要ならSnowflake秘密鍵の参照先 |
| 設定画面 | 使用するメタデータテンプレート、AI分類、並列数、処理ログの出力先 |
| 「新しい移行」 | 移行名、Macの移行元フォルダー、Boxの移行先、移行ごとの詳細オプション |

設定画面の「認証情報」は現在の認証方式の表示。接続テストの結果ではなく、認証情報の編集欄でもない。
移行元とBoxの配置先は共通設定に登録しない。移行を作成するときに選択する。

## メタデータ

「使用するテンプレート」でBoxの既存テンプレートを複数選択し、保存する。
AI分類が有効なら、文書本文と候補の名前・項目からテンプレートを選び、項目も自動抽出する。
承認画面ではテンプレートを変更でき、その場合も自動抽出する。値は必要なときだけ詳細を開いて確認する。
設定保存だけで処理済みのファイルを再分類することはない。
対応する項目の型と事前準備は[書類別メタデータ](metadata-templates.md)を参照。
...
詳細設定の保存値はSQLiteのruntime_settings（schema 5で追加）に保存され、再起動後も維持される。
