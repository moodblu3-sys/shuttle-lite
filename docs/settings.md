...
転送・AI分類・承認後の配置には別々の処理枠があり、完了した枠へ次のファイルを投入する。
転送は上記のファイル並列数、AI分類と配置はそれぞれ最大2ファイル。
分割転送の枠は処理中の全ファイルで共有する。
操作要求（一時停止・承認・再スキャンなど）や設定変更を受けた場合は、新規処理の投入を止め、
実行中の処理ステップが終了してから反映する。送信中のHTTPリクエストは強制中断しない。
長時間の転送中もジョブの実行権を定期更新し、別ワーカーによる二重実行を防ぐ。
