...
- 2026-09-23の承認・完了画面改善を含め、自動テスト423件、fake Box検証22項目、型チェック・lint・本番ビルド成功。現行schemaは10。
...
- 承認一覧の列を統一し、状態フィルターを追加。配置先の下書きも含めて移行全体を絞り込む。詳細は選択時だけ開く。
- 完了画面は成功・失敗・除外とBox・レポートへのリンクを中心に表示。抽出失敗・処理中のファイルは一括承認に含めない。
- UIの操作・履歴・下書きは自動テスト済み。今回の環境ではブラウザープロセスを起動できず、1440/1280pxでの見た目確認はMac側に残る。
- 他ツールとの比較と追加開発の優先順位は [migration-tool-comparison.md](migration-tool-comparison.md)。提案した追加機能は未実装。
